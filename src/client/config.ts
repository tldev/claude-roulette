...
export interface LocalConfig { identities: Record<string, string>; profile: Profile; consent: boolean }
...
export function loadConfig(directory = configDirectory()): LocalConfig {
  try {
    ...
    return { identities, profile: ProfileSchema.parse(value.profile ?? defaultProfile), consent: value.consent === true };
  } catch (error) {
    ...
    return { identities: {}, profile: defaultProfile, consent: false };
  }
}
